 ...
 * 字符串转 ASCII 码（十进制）
 */
export function stringToAsciiDecimal(input: string): string {
 ...
 * 字符串转 ASCII 码（十六进制）
 */
export function stringToAsciiHex(input: string): string {
 ...
 * 字符串转 ASCII 码（八进制）
 */
export function stringToAsciiOctal(input: string): string {
 ...
 * 字符串转 ASCII 码（二进制）
 */
export function stringToAsciiBinary(input: string): string {
 ...
 * ASCII 码转字符串（十进制）
 */
export function asciiDecimalToString(input: string): string {
 ...
 * ASCII 码转字符串（十六进制）
 */
export function asciiHexToString(input: string): string {
  const codes = input.trim().split(/\s+/).filter(s => s.length > 0)
  return codes.map(code => {
 ...
 * ASCII 码转字符串（八进制）
 */
export function asciiOctalToString(input: string): string {
  const codes = input.trim().split(/\s+/).filter(s => s.length > 0)
  return codes.map(code => {
 ...
 * ASCII 码转字符串（二进制）
 */
export function asciiBinaryToString(input: string): string {
  const codes = input.trim().split(/\s+/).filter(s => s.length > 0)
  return codes.map(code => {
 ...
 * 字符串转 ASCII（支持多种格式）
 */
export function stringToAscii(input: string, format: AsciiFormat = 'decimal'): AsciiResult {
  try {
    if (!input) {
 ...
 * ASCII 转字符串（支持多种格式）
 */
export function asciiToString(input: string, format: AsciiFormat = 'decimal'): AsciiResult {
  try {
    if (!input || !input.trim()) {
      return { success: false, result: '', error: '输入不能为空' }
    }
 ...
 * 自动检测格式并转换
 */
export function asciiAutoDetect(input: string): AsciiResult {
  try {
    if (!input || !input.trim()) {
      return { success: false, result: '', error: '输入不能为空' }
    }
    ...
    // 检测格式
    let format: AsciiFormat = 'decimal'
